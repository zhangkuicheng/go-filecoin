...
func TestProcessBlockVMErrors(t *testing.T) {
	...
	assert.Len(receipts, 1)
	assert.Contains(string(receipts[0].ReturnBytes()), "boom")
...
func TestProcessBlockParamsLengthError(t *testing.T) {
	...
	assert.Contains(string(r.ReturnBytes()), "invalid params: expected 2 parameters, but got 1")
}
...
func TestProcessBlockParamsError(t *testing.T) {
	...
	assert.Contains(string(r.ReturnBytes()), "invalid params: malformed stream")
}
