...
func block(require *require.Assertions, height int, parentCid *cid.Cid, msg string) *types.Block {
...
func TestTipSet(t *testing.T) {
	...
	b1 := block(require, 1, cid1, "1")
	b2 := block(require, 1, cid1, "2")
	b3 := block(require, 1, cid1, "3")
...
func TestTipIndex(t *testing.T) {
	...
	cidGetter := types.NewCidForTestGetter()
	cid1 := cidGetter()
	b1 := block(require, 42, cid1, "foo")
	idx.addBlock(b1)
	contains(b1, 1, 1, 1)

	b2 := block(require, 42, cid1, "bar")
	idx.addBlock(b2)
	contains(b2, 1, 1, 2)

	cid3 := cidGetter()
	b3 := block(require, 42, cid3, "hot")
	idx.addBlock(b3)
	contains(b3, 1, 2, 1)

	cid4 := cidGetter()
	b4 := block(require, 43, cid4, "monkey")
	idx.addBlock(b4)
	contains(b4, 2, 1, 1)
}
