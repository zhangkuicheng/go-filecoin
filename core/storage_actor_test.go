...
func TestStorageMarketCreateMiner(t *testing.T) {
	...
	outAddr, err := types.NewAddressFromBytes(receipt.ReturnBytes())
	assert.NoError(err)
...
func TestStorageMarketCreateMinerPledgeTooLow(t *testing.T) {
	...
	receipt, err := ApplyMessage(ctx, st, msg)
	assert.NoError(err)
	assert.Contains(string(receipt.ReturnBytes()), ErrPledgeTooLow.Error())
}
...
func TestStorageMarketAddBid(t *testing.T) {
	...
	assert.Equal(types.NewTokenAmount(0), types.NewTokenAmountFromBytes(receipt.ReturnBytes()))
	...
	assert.Equal(uint8(0), receipt.ExitCode)
	assert.Equal(types.NewTokenAmount(1), types.NewTokenAmountFromBytes(receipt.ReturnBytes()))
	...
	receipt, err = ApplyMessage(ctx, st, msg)
	assert.NoError(err)
	assert.Contains(string(receipt.ReturnBytes()), "must send price * size funds to create bid")
}
