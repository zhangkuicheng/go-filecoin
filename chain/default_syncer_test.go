...
func TestSyncIgnoreLightFork(t *testing.T) {
	...
	err = syncer.HandleNewBlocks(ctx, forkCids1)
...
func (pt *powerTableForWidenTest) Miner(ctx context.Context, st state.Tree, bs bstore.Blockstore, mAddr types.Address) (uint64, error) {
	return uint64(25), nil
}

func (pt *powerTableForWidenTest) HasPower(ctx context.Context, st state.Tree, bs bstore.Blockstore, mAddr types.Address) bool {
	return true
}
...
func TestTipSetWeightDeep(t *testing.T) {
	...
	addr0, block, nonce, err := CreateMinerWithPower(ctx, t, syncer, weightDeepGenBlk, mockSigner, 0, mockSigner.Addresses[0], nil, cst, bs, weightDeepGenCid)
	require.NoError(err)
