...
func MakeNodesUnstarted(t *testing.T, n int, offlineMode bool, mockMineMode bool, options ...func(c *Config) error) []*Node {
	...
	var out []*Node
	for i := 0; i < n; i++ {
		nd := genNode(t, offlineMode, mockMineMode, consensus.InitGenesis, nil, options)
		out = append(out, nd)
	}
...
func genNode(t *testing.T, offlineMode bool, mockMineMode bool, gif consensus.GenesisInitFunc, initopts []InitOpt, options []func(c *Config) error) *Node {
	...
	if mockMineMode {
		...
		newSyncer := chain.NewDefaultSyncer(nd.OnlineStore, nd.CborStore, newCon, nd.ChainReader)
		nd.Syncer = newSyncer
		nd.Consensus = newCon
	}
...
func requireResetNodeGen(require *require.Assertions, node *Node, gif consensus.GenesisInitFunc) {
