...
func NewVMContext(from, to *types.Actor, msg *types.Message, st *state.CachedTree, s Stage, bh *types.BlockHeight) *Context {
	return &Context{
		from:        from,
		to:          to,
		message:     msg,
...
func (ctx *Context) Send(to types.Address, method string, value *types.AttoFIL, params []interface{}) ([][]byte, uint8, error) {
	...
	innerCtx := NewVMContext(fromActor, toActor, msg, ctx.state, ctx.stage, ctx.blockHeight)
