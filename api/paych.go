...
type Paych interface {
	Create(ctx context.Context, fromAddr, target address.Address, eol *chain.BlockHeight, amount *types.AttoFIL) (*cid.Cid, error)
	...
	Extend(ctx context.Context, fromAddr address.Address, channel *types.ChannelID, eol *chain.BlockHeight, amount *types.AttoFIL) (*cid.Cid, error)
}
