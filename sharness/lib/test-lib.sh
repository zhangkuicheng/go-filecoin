...
BIN_NAME=go-filecoin

# add current directory to path, for ipfs tool.
if test "$MAKE_SKIP_PATH" != "1"; then
  BIN=$(cd .. && echo `pwd`/bin)
  BIN2=$(cd ../ && echo `pwd`/sharness/bin)
  PATH=${BIN2}:${BIN}:${PATH}

  # assert the `go-filecoin` we're using is the right one.
  if test `which $BIN_NAME` != ${BIN2}/$BIN_NAME; then
	  which $BIN_NAME
	  echo $BIN2
	  ls $BIN2
    echo >&2 "Cannot find the tests' local filecoin tool."
    echo >&2 "Please check test and filecoin tool installation."
    exit 1
  fi
fi
...
# is not always used.
export FIL_PATH="$(pwd)/filecoin-dir"
...
test_launch_filecoin_daemon() {
  ...
  test_expect_success "'filecoin daemon' succeeds" '
    go-filecoin daemon $args >actual_daemon 2>daemon_err &
  '

  # wait for api file to show up
  # TODO: this is mostly just copy pasted from ipfs, fix for real for filecoin
  test_expect_success "api file shows up" '
    test_wait_for_file 20 100ms "$FIL_PATH/filecoin-daemon-lock"
  '
...
test_kill_filecoin_daemon() {
  ...
  test_expect_success "'go-filecoin daemon' is still running" '
    kill -0 $FIL_PID
  '

  test_expect_success "'go-filecoin daemon' can be killed" '
    test_kill_repeat_10_sec $FIL_PID
  '
}
