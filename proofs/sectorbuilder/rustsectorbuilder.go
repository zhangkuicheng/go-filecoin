...
func (sb *RustSectorBuilder) AddPiece(ctx context.Context, pieceRef cid.Cid, pieceSize uint64, pieceReader io.Reader) (sectorID uint64, retErr error) {
	...
	sinkDirPath, sinkPath, err := byteSinkPath(pieceRef)
	if err != nil {
		return 0, err
	}
	defer os.RemoveAll(sinkDirPath)

	sink, err := bytesink.NewFifo(sinkPath)
	if err != nil {
		return 0, err
	}
	...
	go func() {
		...
		defer func() {
			...
			err = sink.Remove()
			if err != nil {
				log.Warningf("failed to remove sink: %s", err)
			}
		}()
	...
	go func() {
		...
		if resPtr.status_code != 0 {
			...
			log.Errorf(msg, C.GoString(resPtr.error_msg), sinkPath)
	...
	select {
	case <-time.After(MaxTimeToWriteBytesToSink):
		errStr := "timed out waiting for bytes to be streamed"
		strFmt := "%s (sinkPath=%s, timeout=%s)"
		log.Errorf(strFmt, errStr, sinkPath, MaxTimeToWriteBytesToSink)

		return 0, errors.New(errStr)
	case <-ctx.Done():
		errStr := "context canceled while streaming piece-byte"
		strFmt := "%s (sinkPath=%s)"
		log.Errorf(strFmt, errStr, sinkPath)
	...
	case sectorID := <-sectorIDCh:
		...
		log.Infof("add piece complete (pieceRef=%s, sectorID=%d, sinkPath=%s)", pieceRef.String(), sectorID, sinkPath)
...
func byteSinkPath(pieceRef cid.Cid) (string, string, error) {
	tmpDir, err := ioutil.TempDir("", "")
	if err != nil {
		return "", "", errors.Wrap(err, "failed to create temp dir")
	}

	return tmpDir, filepath.Join(tmpDir, pieceRef.String()), nil
}
