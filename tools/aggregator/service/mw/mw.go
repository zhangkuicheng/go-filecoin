...
var log = logging.Logger("mw")

// Taken from go-log/writer
...
func (bw *bufWriter) die() {
	bw.deathLock.Lock()
	bw.dead = true
	bw.writer.Close() // nolint: errcheck
	bw.deathLock.Unlock()
}
	...
	go func() {
		for b := range nextCh {
			...
			if err != nil {
				...
				// wouldn't want to log here as it could cause an infinite loop
				bw.die()
				return
			}
		}
	...
	for {
		...
		case b, ok := <-incoming:
			...
			if bufsize > MaxWriterBuffer {
				...
				nextCh = nil
				// explicitly keep going here to drain incoming
			}
		case nextCh <- nextMsg:
			nextMsg = nil
			if len(buffered) > 0 {
