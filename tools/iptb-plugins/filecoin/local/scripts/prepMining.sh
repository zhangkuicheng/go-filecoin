...
iptb="$GOPATH/src/github.com/filecoin-project/go-filecoin/tools/iptb/iptb"
...
printf "Setting up heartbeat"
for i in `seq 1 $J`
do
  $iptb run -- go-filecoin config heartbeat.nickname "iptb"
  $iptb run -- go-filecoin config heartbeat.beatTarget "/ip4/127.0.0.1/tcp/9081/ipfs/QmVR3UFv588pSu8AxSw9C6DrMHiUFkWwdty8ajgPvtWaGU"
done
...
do
    ...
    msgCidRaw=$($iptb run 0 -- go-filecoin message send --price 0 --limit 0 --from "$minerOwner" --value 100 "$nodeAddr")
    ...
    newMinerAddr=$($iptb run "$i" -- go-filecoin miner create --price 0 --limit 0 10 10 | tail -n +3)
